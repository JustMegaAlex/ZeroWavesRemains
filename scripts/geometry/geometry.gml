...
function Vec2(xx, yy, is_polar=false) constructor {
	...
	approach2 = function(to, ratio, treshold=0.003) {
        var diff = to.sub_(self)
        var len = diff.len()
        var sp = len * ratio
        if sp < treshold {
...
function Line(_xst, _yst, _xend, _yend) constructor {
	...
	static set_point_on = function(m, point) {
		var xx = xst + (xend - xst) * m
		var yy = yst + (yend - yst) * m
		point.set(xx, yy)
	}
...
function LineIntersectionPoint(l1, l2, segment, point=undefined) {
    var m = LineIntersection(l1, l2, segment)
    if point != undefined {
        l1.set_point_on(m, point)
        return point
    }
    return l1.get_point_on(m)
}
